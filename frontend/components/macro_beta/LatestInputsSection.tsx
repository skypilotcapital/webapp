        ...
        {data && (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 text-sm">
            ...
            <div className="rounded-2xl border border-slate-100 p-4 bg-slate-50">
              ...
              <p className="text-slate-600 mt-1">Level: {data.sp500_level?.toFixed(2) ?? '—'}</p>
              <p className="text-slate-500">Date: {data.sp500_date ?? '—'}</p>
            </div>
            <div className="rounded-2xl border border-slate-100 p-4 bg-slate-50">
              <p className="font-bold text-slate-800">BBB Credit</p>
              <p className="text-slate-600 mt-1">OAS: {data.bbb_oas_decimal?.toFixed(4) ?? '—'}</p>
