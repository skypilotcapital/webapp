    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between pb-3 border-b border-black mb-4">
          <h2 className="text-lg font-bold text-black uppercase tracking-tight">Pipeline Run Log</h2>
          {updatedAt && (
        ...
        {data && data.length > 0 && (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-200">
              {data.map((row) => (
                <tr key={`${row.flow}.${row.step}`} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-mono text-slate-600 text-xs">{row.flow}</td>
                  <td className="px-6 py-4 font-mono text-black font-medium text-xs">{row.step}</td>
