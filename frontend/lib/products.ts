...
// ⚠️ CURRENTLY UNREACHABLE: every product above carries an explicit `fullLabel`, so
// `portfolios/[strategy]` short-circuits before this runs. It is kept because the call site still
// exists, and it is kept CORRECT because an unreachable function that returns a plausible-but-wrong
// label is worse than one that throws — the page would render someone else's book.
//
// The vintage cases must come FIRST. A v4 in-sample label is `..._LOCKED_relcap_ewma60_rc5_...` and
// its modeled-paper twin is `..._LOCKED_relcap_ewma60_v4_full_rc5_...`; the generic `_LOCKED_relcap_`
// rule below would splice `_full` in the wrong place and silently miss.
export const fullLabelOf = (prodLabel: string) =>
  prodLabel.includes('_LOCKED_relcap_ewma60_rc')
    ? prodLabel.replace('_LOCKED_relcap_ewma60_rc', '_LOCKED_relcap_ewma60_v4_full_rc')
