...
function CollapsibleCorrelationMatrix() {
  ...
  return (
    <div className="panel">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-4 py-3 font-semibold text-xs text-[var(--tx-mut)] hover:bg-[rgba(45,212,191,0.06)] rounded-xl transition-colors"
      >
        <span className="flex items-center gap-2">
...
export default function ModelsPage() {
  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      {/* Header + info boxes — frozen (never scroll), condensed to a single row */}
      <div className="flex-none">
        <div className="mb-2 flex items-baseline gap-2.5 flex-wrap">
          <h1 className="text-base font-bold text-[var(--tx)] tracking-tight">Alpha Model Analysis</h1>
          ...
          <span className="text-[10px] text-[var(--tx-dim)] font-medium uppercase tracking-wider">Walk-Forward Backtest · RF / LightGBM / Lasso / ensembles</span>
