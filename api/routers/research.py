...
def get_model_scorecard(universe: str = "sp500"):
    ...
    universe: 'sp500' (default) returns the current S&P 500 generation — N000-N015 (N*).
              'russell2500' returns the current SMID generation — NR001-NR015 (NR*).

    The legacy M*/MR* zoo (M001-M019, MR001-MR020) was built on superseded/pre-audit data
    and has been removed entirely — not shown here and purged from the database. The current
    production generation is N* (S&P 500) and NR* (Russell 2500), both on the corrected
    clean-2026-06 data + 53-factor superset.

    sector_*_monthly is the preferred significance metric.
    sector_*_panel pools all date×sector ICs (supplementary only).
    """
    # Current generation only. SP500 = N* (excluding the R-prefixed NR*); R2500 = NR*.
    if universe == "russell2500":
        universe_filter = "model_id LIKE 'NR%'"
    else:
        universe_filter = "(model_id LIKE 'N%' AND model_id NOT LIKE 'NR%')"
